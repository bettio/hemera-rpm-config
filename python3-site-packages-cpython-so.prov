...
filelist=`grep -e '[a-zA-Z_][a-zA-Z0-9_]*\.cpython-3[0-9][a-zA-Z]\.so$' | sed "s/\/usr\/lib\/python3\.4\/site-packages\///g" | sed "s/\/__init__\.cpython-3[0-9][a-zA-Z]\.so//g" | sed "s/\.cpython-3[0-9][a-zA-Z].so//g" | sed "s/\//./g"`
